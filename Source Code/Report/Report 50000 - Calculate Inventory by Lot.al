report 50000 "Calculate Inventory By Lot"
{
    Caption = 'Calculate Inventory by Lot';
    ProcessingOnly = true;
        ...
        dataitem(Item; Item)
        {
                ...
                begin
                    ...
                    if not SkipCycleSKU("Location Code", "Item No.", "Variant Code") then
                        if ByBin then begin
                            ...
                            UpdateBufferWithTracking("Lot No.", "Serial No.", '', Quantity, True); //BIS
                end;
            ...
            dataitem("Warehouse Entry"; "Warehouse Entry")
            {
                ...
                begin
                    ...
                    //BIS++
                    TempQuantityOnHandBuffer."Lot No." := "Lot No.";
                    TempQuantityOnHandBuffer."Serial No." := "Serial No.";
                    //BIS--
    ...
    procedure InsertItemJnlLine(LotNo: Code[20]; Serial: Code[20]; ItemNo: Code[20]; VariantCode2: Code[10]; DimEntryNo2: Integer; BinCode2: Code[20]; Quantity2: Decimal; PhysInvQuantity: Decimal)
    //BIS Add Lot and Serial
    var
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            if ShouldInsertItemJnlLine then begin
                ...
                //BIS++
                ItemJnlLine.Description := 'Lot/Serial ' + LotNo + Serial;
                //BIS--
    ...
    begin
        TempQuantityOnHandBuffer.Reset();
        TempQuantityOnHandBuffer.SetRange(TempQuantityOnHandBuffer."Item No.", ItemNo);
        TempQuantityOnHandBuffer.SetRange(TempQuantityOnHandBuffer."Location Code", LocationCode);
        TempQuantityOnHandBuffer.SetRange(TempQuantityOnHandBuffer."Variant Code", VariantCode);
    ...
    //BIS++
    local procedure RetrieveBufferwithLotSerial(Lot: Code[20]; Serial: Code[20]; BinCode: Code[20]; DimEntryNo: Integer): Boolean
    begin
        TempQuantityOnHandBuffer.Reset();
        TempQuantityOnHandBuffer."Item No." := "Item Ledger Entry"."Item No.";
        TempQuantityOnHandBuffer."Variant Code" := "Item Ledger Entry"."Variant Code";
        ...
        TempQuantityOnHandBuffer."Bin Code" := BinCode;
        //BIS++
        TempQuantityOnHandBuffer."Lot No." := Lot;
        TempQuantityOnHandBuffer."Serial No." := Serial;
        //BIS--
        OnRetrieveBufferOnBeforeFind(TempQuantityOnHandBuffer, "Item Ledger Entry");
        exit(TempQuantityOnHandBuffer.Find());
    ...
    local procedure UpdateBufferWithTracking(Lot: Code[20]; Serial: Code[20]; BinCode: Code[20]; NewQuantity: Decimal; CalledFromItemLedgerEntry: Boolean)
    var
        DimEntryNo: Integer;
    begin
        if not HasNewQuantity(NewQuantity) then
        ...
        if RetrieveBufferwithLotSerial(Lot, Serial, BinCode, DimEntryNo) then begin
            TempQuantityOnHandBuffer.Quantity := TempQuantityOnHandBuffer.Quantity + NewQuantity;
            //OnUpdateBufferOnBeforeModify(TempQuantityOnHandBuffer, CalledFromItemLedgerEntry);
            TempQuantityOnHandBuffer.Modify();
        end else begin
            ...
            repeat
                ...
                if (NegQty - TempQuantityOnHandBuffer.Quantity <> TempQuantityOnHandBuffer.Quantity - PosQty) or ItemTrackingSplit then begin
                    ...
                    if (PosQty <> 0) or AdjustPosQty then
                        InsertItemJnlLine(
                            TempQuantityOnHandBuffer."Lot No.", TempQuantityOnHandBuffer."Serial No.", //BIS++
                    ...
                    if NegQty <> 0 then begin
                        ...
                        InsertItemJnlLine(
                            TempQuantityOnHandBuffer."Lot No.", TempQuantityOnHandBuffer."Serial No.",
                          TempQuantityOnHandBuffer."Item No.", TempQuantityOnHandBuffer."Variant Code", TempQuantityOnHandBuffer."Dimension Entry No.",
    ...
    begin
        DefaultDimension.SetRange(DefaultDimension."No.", TempQuantityOnHandBuffer."Item No.");
